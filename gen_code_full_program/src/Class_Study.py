...
class CStudy:
    ...
    def CY_serie_found(self, ObjtImg):
        ...
        if ObjtImg.D02SeNu not in self.SeNuLst:
            ...
            self.AllSers.append(
                CSerie(ObjtImg.D02SeNu, self.StuItem, var_aux0, self.resolution)
            )
    ...
    def CY_geom_qa(self, serie_array):
        ...
        for i in serie_array:
            for j in serie_array:
                var_aux1 = self.AllSers[i].S04SlTk / self.AllSers[j].S04SlTk
                var_aux2 = (
                    self.AllSers[i].S04RwNb
                    * self.AllSers[j].S04ClNb
                    / (self.AllSers[i].S04ClNb * self.AllSers[j].S04RwNb)
                )
                var_aux3 = (
                    self.AllSers[i].AllImag[0].D04Slic
                    * self.AllSers[i].AllImag[1].D04Slic
                    / (
                        self.AllSers[j].AllImag[0].D04Slic
                        * self.AllSers[j].AllImag[1].D04Slic
                    )
                )
                var_aux4 = len(self.AllSers[i].AllImag) / len(self.AllSers[j].AllImag)
    ...
    def CYPrtInfo(self):
        if self.StImRef != -4:
            print(
                "\n************************************\nNombre:",
                self.StImRef.D01Name,
                "    Estudio:",
                self.StImRef.D01StTm,
            )
        else:
            print(
                "\n************************************\nNombre:", self.StImRef.D01Name
            )
        if self.StImRef.D01StDc != -4:
            print("Descripción:", self.StImRef.D01StDc)
    ...
    def CY_new_serie(self, ref_ser1, ref_ser2, new_seri, operaSr):
        ...
        var_aux0 = len(self.SeNuLst) - 1
        self.AllSers.append(
            CSerie(new_seri, self.StuItem, var_aux0, self.resolution)
        )  # Crea serie nueva
        var_aux4 = len(self.AllSers) - 1
        var_aux5 = self.AllSers[var_aux1].refImag  # Determina imagen de referencia
        self.AllSers[var_aux4].CSRCopFea(
            var_aux5, 1
        )  # Llena las caracteristicas de la nueva serie

        # Calcula la nueva serie
        var_aux5 = (
            self.AllSers[var_aux1].AllImag[0].ImgData.shape
        )  # Tamaño de la serie de salida
        ...
        for i in range(len(self.AllSers[var_aux1].AllImag)):  # Para todas las img...
            var_aux6 = (
                self.AllSers[var_aux2].AllImag[i].ImgData
            )  # Se iguala a la mas grande
            var_aux6 = cv2.resize(var_aux6, var_aux5)
            if operaSr == 0:
                pass  # Redimensionar a menor imagen
            elif operaSr == 1:  # Sumar
                var_aux6 += self.AllSers[var_aux1].AllImag[i].ImgData
                var_aux6 = var_aux6 * 0.5
            elif operaSr == 2:  # Resta
                if var_aux3 == 0:
                    var_aux6 = self.AllSers[var_aux1].AllImag[i].ImgData - var_aux6
                else:
                    var_aux6 -= self.AllSers[var_aux1].AllImag[i].ImgData
                var_aux6 = var_aux6 * 0.5
            elif operaSr == 3:  # Multiplicacion
                var_aux6 *= 0.01 * self.AllSers[var_aux1].AllImag[i].ImgData
                var_aux6 = 10 * np.sqrt(var_aux6)
            ...
            ObjtImg.CIRCopFea(
                self.AllSers[var_aux1].AllImag[i],
                var_aux6,
                new_seri,
                self.StuItem,
                var_aux4,
    ...
    def CYRmSerie(self, entry_type, data_val):
        ...
        if entry_type == 0:
            ...
            raise ValueError("Error: No se elimina serie (CYRmSe).")
        varAllS = self.AllSers[vr_allSer]

        # Se eliminan objetos
        self.AllSers[vr_allSer].CSRmImage(0)  # Elimina imagenes
        self.SeNuLst.remove(vr_serNu)
        self.AllSers.remove(varAllS)  # Elimina serie
        var_aux0 = 0
        for i in CSerie.Instanc:  # Elimina serie de Instanc
            if var_aux0 > 0:
                raise ValueError("Error: Repeticion de serie (CYRmSe).")
            if i.StuItem == self.StuItem and i.S02SeNu == vr_serNu:
                CSerie.Instanc.remove(i)
                var_aux0 += 1
    ...
    def CY_serGeo_proj(self, ref_ser1, ref_ser2, new_seri):
        new_serie = self.CY_new_serie(
            ref_ser1, ref_ser1, new_seri, 5
        )  # New black serie
