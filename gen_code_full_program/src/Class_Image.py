...
class CImage(CDcmRd):  # Hereda de clase DcmRd
    ...
    def CIDescDCM(self, PathAux):
        ...
        if not reader.Read():
            print("Error: No se leen las imágenes (CIDes).")
            sys.exit(0)
        ...
        if not change.Change():
            print("Error: No se descomprime las imágenes (CIDes).")
            sys.exit(0)
        ...
        if not writer.Write():
            print("Error: No se escriben las imágenes (CIDes).")
            sys.exit(0)
    ...
    def CIRCopFea(self, RefImag, ImgMatx, SerNumb, StuItem, SerItem):
        ...
        self.D00CpTp = 0
        self.D01Name = RefImag.D01Name
        self.D01StDc = RefImag.D01StDc
        # self.D00TfSy=-2; self.D00Sour=-2
        # self.D01StTm=-2; self.D01Mnfr=-2
        # self.D01MnMd=-2
        self.D01Mdly = RefImag.D01Mdly
        self.D04SlTk = RefImag.D04SlTk
        self.D04RwNb = RefImag.D04RwNb
        self.D04ClNb = RefImag.D04ClNb
        self.D04PiSp = RefImag.D04PiSp
        self.D04DiCo = RefImag.D04DiCo
        self.D04Slic = RefImag.D04Slic
        self.D04ImPo = RefImag.D04ImPo
        self.D10ExFd = RefImag.D10ExFd
        self.D12AcPl = RefImag.D12AcPl
        self.D02SeNu = SerNumb
        # self.D03AcNb=-2
    ...
    def CI_trans_base(self):
        ...
        self.trns_MatOr = np.array(
            [
                [
                    vec_rwo[i] * self.D04PiSp[0],
                    vec_clo[i] * self.D04PiSp[1],
                    vec_nor[i] * self.D04SlTk,
                    self.D04ImPo[i],
                ]
                for i in range(3)
            ]
    ...
    def CIFTitles(self, PrtType):
        ...
        if PrtType == 0:
            ...
            TitleSt += "Img_"
            varAux0 = str(self.D01Name)
            TitleSt += (
                varAux0[:4] + "_S" + str(self.StuItem) + "_R" + str(self.SerItem) + "_I"
            )
