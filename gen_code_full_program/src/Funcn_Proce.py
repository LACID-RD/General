...
def FP_hist_analysis(hist_data, bins_num, window, transform, a_coef, var_print):
    ...
    for i in np.where(zero_indices)[
        0
    ]:  # zero_indices array where the value is True [0]:1D
        # Slice of count_vec from the start index to the end index.
        var_aux1 = count_vec[
            max(0, i - int(bins_num / 10)) : min(len(count_vec), i + int(bins_num / 10))
        ]
...
def FP_serGeo_proj(ref_img1, ref_img2):
    ...
    plt.show()
    print(
        "Items:", ref_img1.StuItem, ref_img1.SerItem, ref_img2.StuItem, ref_img2.SerItem
    )
    # print()
