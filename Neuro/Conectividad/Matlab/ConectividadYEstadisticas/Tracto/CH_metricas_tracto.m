...
%Cambiar el número de paciente en la primera línea de cada métrica
BC_PAT = BC_14;
...
%Cluster Coefficient
%cluster_coef_PAT = cluster_coef_10;
...
%Distance matrix
%distance_matrix_PAT = distance_matrix_10;
...
%Edges_distance
%edgesdistance_PAT = edges_distance_10;
...
%Global Efficiency
%globalefficiency_PAT = global_efficiency_10;
...
%Local efficiency
%localeff_PAT = local_efficiency_10;
...
for i=1:a
    for j=1:a
        if diferencia_distancematrix(i,j) < 0.05
...
title('Distance matrix con threshold en p<0.05, paciente 01');

edgesdistance_thr = zeros(a,a);
for i=1:a
    for j=1:a
        if diferencia_edgesdistance(i,j) < 0.05
...
colorbar
set(gca,'ColorScale','log'); %escala de colores logarítmica
title('Edges distance con threshold en p<0.05, paciente 01');

BC_threshold=zeros(1,a);
for i=1:a
    if diferencia_BC(i) < 0.05
        BC_threshold(i) = 1;
...
for i=1:a
    if diferencia_clustercoef(i) < 0.05
